...
public class DailyIndexValuesForAllIndices5DaysTest extends AbstractTest{

    private static final Logger logger
            = LoggerFactory.getLogger(DailyIndexValuesForAllIndices5DaysTest.class);

    @Test
    void getIndexForAllIndicesFiveDaysResponseCode200() throws IOException {
        ...
        logger.debug("Формирование мока для GET /indices/v1/daily/5day/294018");
        stubFor(get(urlPathEqualTo("/indices/v1/daily/5day/294018"))
        ...
        HttpGet request = new HttpGet(getBaseUrl() + "/indices/v1/daily/5day/294018");

        HttpResponse response = httpClient.execute(request);

        //then
        verify(getRequestedFor(urlPathEqualTo("/indices/v1/daily/5day/294018")));
        Assertions.assertEquals(200, response.getStatusLine().getStatusCode());

        FiveDay resultFiveDay = objectMapper.readValue(response.getEntity().getContent(), FiveDay.class);
        Assertions.assertEquals("Flight Delays", resultFiveDay.getName());
    }

    @Test
    void getIndexForAllIndicesFiveDaysResponseCode401() throws IOException, URISyntaxException {
